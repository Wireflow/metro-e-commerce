...
export const getIndependantSales = async ({ startDate, endDate }: DateRange = {}) => {
  try {
    ...
    let query = supabase
      .from('independent_salesperson_branch_analytics')
      .select('*')
    ...
    if (error) {
      ...
      throw new Error('Failed to fetch independant salesperson sales data');
    }

    return data;
  } catch (error) {
    console.error('Error in getIndependantSales:', error);
    throw new Error('Failed to fetch independant salesperson sales data');
  }
};
